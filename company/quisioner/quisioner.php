...
?>
        ...
        <form id="asstitle">
            <div class="form-group">
                <div class="col-md-8">
                  <label class="col-md-4 control-label" for="quisionertitle">Quisioner Title:</label>
                  <div class="col-md-8">
                      <input type="hidden" name="COMPANYID" value="<?= $COMPANYID; ?>">
                      <input type="hidden" name="JOBID" value="<?= $cur->JOBID; ?>">
                    <input class="form-control input-sm" id="quisionertitle" name="quisionertitle" placeholder="Quisioner Title"
                      autocomplete="none" />
                  </div>
                </div>
              </div>
...
<script>
    $(document).ready(function(){
        $('#asstitle').submit(function(e){
			...
			$.ajax({
				url:'controller.php?action=quisioner',
				method:'POST',
				data:$(this).serialize(),
				error:err=>{
					console.log(err)
				...
				success:function(resp){
					if(typeof resp != undefined){
						...
						if(resp.status == 1){
							...
							location.replace('index.php?view=form&id='+resp.id)
						}else{
